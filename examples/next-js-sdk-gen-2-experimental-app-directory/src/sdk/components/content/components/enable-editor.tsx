...
type BuilderEditorProps = Omit<
  ContentProps,
  'customComponents' | 'apiVersion' | 'isSsrAbTest' | 'blocksWrapper' | 'blocksWrapperProps'
> & {
...
function EnableEditor(props: BuilderEditorProps) {
  ...
  useEffect(() => {
    evaluateJsCode();
  }, [props.builderContextSignal.content?.data?.jsCode, props.builderContextSignal.rootState]);
  useEffect(() => {
    runHttpRequests();
  }, [props.builderContextSignal.content?.data?.httpRequests]);
  useEffect(() => {
      ...
      {props.builderContextSignal.content ? (
        <>
          <ContentWrapper
            ...
            className={`variant-${props.content?.testVariationId || props.content?.id}`}
          >
            {props.children}
          </ContentWrapper>
        </>
